// The code has been updated to allow dispensing multiple products until the
//  received payment amount is exhausted.
//   It ensures that each product's price is deducted from the payment 
//   and checks stock availability before dispensing.
...
const char* paymentNumber = "01714425692"; // Display payment number for manual payment
...
void processPayment(String message) {
  ...
  while (receivedAmount >= products[currentIndex].price) {
    Serial.println("Sufficient amount. Dispensing product...");
    dispenseProduct();
    receivedAmount -= products[currentIndex].price;

    if (products[currentIndex].stock <= 0) {
      Serial.println("Out of Stock. Stopping dispensing for this product.");
      break;
    }

    delay(1000); // Small delay between dispensing multiple products
  }

  if (receivedAmount < products[currentIndex].price) {
    Serial.println("Insufficient amount for another product.");
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("Payment Remaining");
    display.print("Balance: BDT ");
    display.println(receivedAmount);
    display.display();
  }
}
