...
void loop() {
  if (digitalRead(BUTTON_PREV_PIN) == LOW) {
    debounce(BUTTON_PREV_PIN);
    previousProduct();
  }

  if (digitalRead(BUTTON_OK_PIN) == LOW) {
    debounce(BUTTON_OK_PIN);
    okButton();
  }

  if (digitalRead(BUTTON_NEXT_PIN) == LOW) {
    debounce(BUTTON_NEXT_PIN);
    nextProduct();
  }
...
void dispenseProduct() {
  Product& currentProduct = products[currentIndex];
  if (currentProduct.stock > 0) {
    currentProduct.stock--;  // Decrease stock after successful dispense

    String path = "/transactions/" + String(currentIndex);
    Firebase.setString(firebaseData, path + "/name", currentProduct.name);
    Firebase.setFloat(firebaseData, path + "/price", currentProduct.price);
    Firebase.setInt(firebaseData, path + "/stock", currentProduct.stock);

    displayMessage("Dispensing " + String(currentProduct.name));
    activateRelay(currentIndex);
    delay(2000);

    displayMessage("Transaction Saved!");
    delay(2000);
    updateDisplay();
  } else {
    displayMessage("Out of Stock!");
...
void activateRelay(int index) {
  ...
  delay(3000);  // Activate relay for 3 seconds
...
void logTransaction(const String& message) {
  ...
  if (!Firebase.pushString(firebaseData, path, message)) {
    Serial.println("Error logging transaction: " + String(firebaseData.errorReason()));
  }
}
