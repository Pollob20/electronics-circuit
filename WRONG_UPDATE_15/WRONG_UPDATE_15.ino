...
// HardwareSerial port 1 ব্যবহার করা হচ্ছে SIM900 মডিউলের সাথে যোগাযোগের জন্য
...
unsigned long relayOnTime = 1000;  // Relay ON time in milliseconds
...
void dispenseProduct() {
  ...
  previousMillis = millis();  // Record the time when the relay was turned ON
  ...
  while (relayActive) {
    if (millis() - previousMillis >= relayOnTime) {
      digitalWrite(RELAY_PINS[relayIndex], HIGH);  // Relay OFF
      relayActive = false;  // Stop the relay after 2 seconds
    }
  }
