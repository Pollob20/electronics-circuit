...
const int IR_SENSOR_PINS[] = {20,21,47,48,15,0};
...
void setup() {
  ...
  // Firebase configuration
  firebaseConfig.api_key = "CxP7HFUo77dz6yNGANVBYjuS9ZgMYX3hhYV5PBVw";
  firebaseConfig.database_url = "https://esp32-vanding-maching-default-rtdb.firebaseio.com/";
  ...
  if (Firebase.ready()) {
    ...
    displayMessage("Database Connected");
  } else {
...
void dispenseProduct() {
  ...
  if (!Firebase.setString(firebaseData, path + "/name", currentProduct.name)) {
    Serial.println("Error logging product name");
  }
  if (!Firebase.setFloat(firebaseData, path + "/price", currentProduct.price)) {
    Serial.println("Error logging product price");
  }
  if (!Firebase.setInt(firebaseData, path + "/stock", currentProduct.stock)) {
    Serial.println("Error logging product stock");
  }

  displayMessage("Dispensing " + String(currentProduct.name));
  activateRelay(currentIndex);
  delay(2000);

  // Display success message after data is saved
...
void activateRelay(int index) {
  if (index < 0 || index >= 6) { // Ensure the index is within bounds
    Serial.println("Invalid relay index");
    return;
  }

  Serial.print("Activating relay for product index: ");
  Serial.println(index);

  digitalWrite(RELAY_PINS[index], HIGH); // Turn on the relay
  delay(3000);                           // Wait for 3 seconds
  digitalWrite(RELAY_PINS[index], LOW);  // Turn off the relay
}
...
void logTransaction(const String& message) {
  String path = "/logs/";
  if (!Firebase.pushString(firebaseData, path, message)) {
    Serial.println("Error logging transaction");
  }
}
